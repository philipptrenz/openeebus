...
struct SenderCallSubscribeTestInput {
...
std::ostream& operator<<(std::ostream& os, SenderCallSubscribeTestInput test_input) {
  return os << test_input.description;
}

class SenderCallSubscribeTests : public SenderTestSuite,
                                 public ::testing::WithParamInterface<SenderCallSubscribeTestInput> {};

TEST_P(SenderCallSubscribeTests, SenderCallSubscribeTests) {
  ...
  // Act: Run the CallSubscribe()
  const EebusError ret
      = SEND_CALL_SUBSCRIBE(sender, sender_addr.get(), dest_addr.get(), GetParam().server_feature_type);
...
INSTANTIATE_TEST_SUITE_P(
    SenderCallSubscribeTests,
    SenderCallSubscribeTests,
    ::testing::Values(
        SenderCallSubscribeTestInput{
            ...
            .server_feature_type = kFeatureTypeTypeNodeManagement,
            .msg_cnt             = 0,
            .msg                 = R"({"datagram":[
                                     {"header":[
